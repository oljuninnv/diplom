            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Тьютор</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">HR-менеджер</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($calls as $call)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center cursor-pointer" onclick="showUserModal({{ $call->candidate_id }})">
                                    <img class="h-10 w-10 rounded-full" src="{{ $call->candidate->avatar_url }}" alt="{{ $call->candidate->name }}">
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $call->candidate->name }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $typeClasses[$call->type] ?? 'bg-gray-100 text-gray-800' }}">
                                    {{ $typeLabels[$call->type] ?? $call->type }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($call->tutor)
                                    <div class="flex items-center cursor-pointer" onclick="showUserModal({{ $call->tutor_id }})">
                                        <img class="h-8 w-8 rounded-full" src="{{ $call->tutor->avatar_url }}" alt="{{ $call->tutor->name }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($call->hr_manager)
                                    <div class="flex items-center cursor-pointer" onclick="showUserModal({{ $call->hr_manager_id }})">
                                        <img class="h-8 w-8 rounded-full" src="{{ $call->hr_manager->avatar_url }}" alt="{{ $call->hr_manager->name }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                @if(app('App\Http\Controllers\MeetingController')->canDelete(auth()->user(), $call))
                                    <form action="{{ route('meetings.destroy', $call->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-gray-500">Нет данных для отображения</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <form id="call-form" action="{{ route('meetings.store') }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                @elseunless(auth()->user()->isTutorWorker())
                    <div>
                        <label for="hr-manager-select" class="block text-sm font-medium text-gray-700 mb-1">HR-менеджер *</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        flatpickr("#date", { dateFormat: "Y-m-d", allowInput: true, locale: "ru" });
        flatpickr("#call-date", { dateFormat: "Y-m-d", allowInput: true, locale: "ru", minDate: "today" });
        flatpickr("#call-time", { 
            enableTime: true, 
            noCalendar: true, 
            dateFormat: "H:i", 
